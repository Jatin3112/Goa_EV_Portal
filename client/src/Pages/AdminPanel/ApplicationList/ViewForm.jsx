...
export default function ViewForm({ id }) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/applications/formData/${id}`
      );
  ...
  useEffect(() => {
    const folderName = data.map((row) => row.folderName);
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/applications/files/${folderName}`
      );
  ...
  const downloadFile = async (fileName) => {
    try {
      const response = await axios.get(
        `${baseURL}/applications/files/${folderName}/${fileName}`,
        {
          responseType: "blob",
        }
      );
              ...
              <CardContent key={row.id}>
                ...
                >
                  ...
                  <CardContent>
                    <Stack direction="row" m={2} sx={flexItems}>
                      ...
                      <Typography variant="body1">{`${row.resgistrationNumber}`}</Typography>
                    </Stack>
                    <Divider />
                    <Stack direction="row" m={2} sx={flexItems}>
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <Divider />
                    <Stack direction="row" m={2} sx={flexItems}>
                      <Typography variant="body1">Vehicle RC</Typography>
                      <Box>
                        <Typography variant="body1">{`${row.vehicleRCNumber}`}</Typography>
                        {files.length > 0 && (
